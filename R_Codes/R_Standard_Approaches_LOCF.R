...
bgevsnum_range <- 2:12
month_range <- c(2, 3, 4, 5, 6, 9, 12, 15, 18, 21, 24)

template <- expand.grid(
  USUBJID = unique(dat_whole$USUBJID),
  BGEVSNUM = bgevsnum_range
)

template$Month <- as.character(factor(template$BGEVSNUM, levels = 2:12, labels = c(2, 3, 4, 5, 6, 9, 12, 15, 18, 21, 24)))
template$Month <- as.numeric(template$Month)

template$Month_scaled <- template$Month * (1/24)

dat_whole <- template %>%
  left_join(dat_whole, by = c("USUBJID", "BGEVSNUM", "Month", "Month_scaled"))

dat_whole <- dat_whole %>%
  group_by(USUBJID) %>%
  arrange(USUBJID) %>%  
  fill(TRAC, TruGraf, two_year_rej, .direction = "downup")
